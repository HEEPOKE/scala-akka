...
class UserController {
  def getAllUsers: Route = {
    onSuccess(UserRepository.getAllUsers) { users =>
      complete(users)
    }
  }
  ...
  def getUser(id: Long): Route = {
    onSuccess(UserRepository.getUser(id)) {
      ...
      case None =>
        complete((StatusCodes.NotFound, Map("message" -> "User not found")))
    }
  }
  ...
  def updateUser(id: Long, user: UserModel): Route = {
    onSuccess(
      ...
      case numRows if numRows > 0 =>
        complete((StatusCodes.OK, Map("message" -> "User updated")))
      case _ =>
        complete((StatusCodes.NotFound, Map("message" -> "User not found")))
    }
  }
  ...
  def deleteUser(id: Long): Route = {
    onSuccess(UserRepository.deleteUser(id)) {
      case numRows if numRows > 0 =>
        complete((StatusCodes.OK, Map("message" -> "User deleted")))
      case _ =>
        complete((StatusCodes.NotFound, Map("message" -> "User not found")))
    }
  }
